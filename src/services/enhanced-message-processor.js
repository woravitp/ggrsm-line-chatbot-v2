...
const handleQuotationRequest = async (parameters, message, session) => {
      try {
              ...
              return `ขออภัย เกิดข้อผิดพลาดในการจัดทำใบเสนอราคา กรุณาติดต่อทีมขายโดยตรงที่ ${CONTACT_PHONE}`;
      }
};
...
const generateQuotation = async (productName) => {
      ...
      q += `✅ รับประกัน 2 ปี\n`;
      q += `✅ บริการหลังการขาย\n\n`;
      q += `📞 สอบถามเพิ่มเติม: ${CONTACT_PHONE}\n`;
...
const getDefaultReply = () =>
      `ขออภัย ระบบขัดข้องชั่วคราว กรุณาติดต่อทีมงานโดยตรงที่ ${CONTACT_PHONE} ครับ`;
